...
export function buildWhatsAppLink(productDescription) {
  const msg = encodeURIComponent(
    `Hi! I'm interested in the ${productDescription} I saw on your website. Can you tell me more about it?`
  );
...
export function WhatsAppIcon({ size = 16, style }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" width={size} height={size} style={style}>
      <path d={WA_PATH} />
    </svg>
  );
}

const FALLBACK_IMG = '/assets/placeholder-dark.jpg';
    ...
    >
      ...
      <div className="card__content" style={{ minHeight: '150px', display: 'flex', flexDirection: 'column', padding: '12px' }}>
        ...
        <div style={{ marginTop: 'auto' }} onClick={(e) => e.stopPropagation()}>
          ...
          <a
            href={buildWhatsAppLink(product.description || product.name)}
            target="_blank"
            rel="noopener noreferrer"
            className="pill whatsapp small"
          >
            <WhatsAppIcon size={14} style={{ marginRight: '4px' }} />
            WhatsApp
          </a>
        </div>
      </div>
          ...
          <div className="product-modal__info">
            ...
            <div className="product-modal__actions">
              ...
              <a href={waLink} target="_blank" rel="noopener noreferrer" className="pill whatsapp">
                <WhatsAppIcon size={16} style={{ marginRight: '6px' }} />
                Ask on WhatsApp
              </a>
            </div>
          </div>
